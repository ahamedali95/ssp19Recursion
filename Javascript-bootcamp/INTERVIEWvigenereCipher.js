...
//get its current position in alphabet
//get key sequence
//add ksey sequence to current position
//get element in new position from alphabet
...
function vigenereCipher(string, keySequence, alphabet) {
  ...
  for (var i = 0; i < string.length; i++) {
    ...
    var currentPosition = alphabet.indexOf(currentChar);
    var shiftBy = keySequence[i] % keySequence.length;
    var newposition = currentPosition + shiftBy;
    var newChar = alphabet[newposition % alphabet.length];
    encrypted += newChar;
  }
...
function vigenereCipher(string, keySequence, alphabet){
    ...
    for(let i = 0; i < string.length; i++){
        ...
        var shiftBy = keySequence[i % keySequence.length];
        var newChar = alphabet[(position + shiftBy) % alphabet.length];
